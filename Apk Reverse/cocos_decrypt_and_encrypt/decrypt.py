...
from xxtea import encrypt_file

OUTPUT_PATH = "./output"
OUTPUT_ENCRYPT_PATH = "./output_encrypt"
...
def start_encrypt(so_file, src_path):
    sign = get_sign(src_path)
    if sign == "":
        print
        "error: src_path contains plain text files."
        return
    key_list = get_key_list(sign, so_file)
    if len(key_list) == 0:
        print
        "error: no valid key, check it manual."
        return
    global OUTPUT_PATH
    for key in key_list:
        print
        "try use key %s to decrypt" % (key)
        if os.path.exists(OUTPUT_PATH):
            shutil.rmtree(OUTPUT_PATH)
        os.mkdir(OUTPUT_PATH)
        ...
        for parent, folders, files in os.walk(src_path):
            ...
            for file in files:
                full_path = os.path.join(parent, file)
                target_path = full_path.replace(src_path, OUTPUT_PATH)
                target_path = target_path.replace("lua", "luac")
                is_right_key = encrypt_file(full_path, key, target_path, sign)
                if not is_right_key:
                    print
                    "invalid key %s on file %s" % (key, full_path)
                    break
            if not is_right_key:
                break
        if is_right_key:
